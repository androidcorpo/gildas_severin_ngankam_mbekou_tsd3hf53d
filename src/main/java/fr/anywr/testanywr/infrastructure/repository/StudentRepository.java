...
public interface StudentRepository extends JpaRepository<Student, Long> {
  ...
  @Query(
      "select s from Student s join s.classRoom c join c.teacher t where c.name =?1 and (concat(t.firstName, t.lastName) like %?2%)")
  Page<Student> findStudentByClassRoomNameAndTeacherFullName(String classRoomName, String teacherFullName, Pageable pageable);
  @Query(
      "select s from Student s join s.classRoom c join c.teacher t where c.name =?1 or (concat(t.firstName, t.lastName) like %?2%)")
